...
Use these exact city names in your `areas_of_interest` configuration. Write them exactly as the Home Front Command defines them - for example, don’t write `תל אביב`, instead write `תל אביב - מרכז העיר`.
